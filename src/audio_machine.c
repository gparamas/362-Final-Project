// // Core0: keypad polling + on-demand MP3 decode.
// // Core1: ping-pong buffer refill.
 
// #include <stdlib.h>
// #include <stdint.h>
// #include <stdbool.h>
// #include <stdio.h>
// #include <string.h>
 
// #include "pico/stdlib.h"
// #include "pico/multicore.h"
// #include "hardware/pwm.h"
// #include "hardware/dma.h"
// #include "hardware/irq.h"
// #include "hardware/clocks.h"
 
// #include "support.h"        // defines RATE 44100
// #include "queue.h"
// #include "audio_assets.h" 

// #include "kick_pcm.h"
// #include "snare_pcm.h"
// #include "hat_pcm.h"
// // #include "snare2_pcm.h"
// #include "sad_sample_pcm.h" 
// // ---------------------------------------------------------------------------
// // Config
// // ---------------------------------------------------------------------------
// #define AUDIO_GPIO   36u
// #define BUF_SAMPLES  2048u
 
// // ---------------------------------------------------------------------------
// // Voice state  (written by core0, read by core1's fill_buf)
// // ---------------------------------------------------------------------------
// typedef struct {
//     int16_t          *pcm;    // decoded samples in RAM; NULL = not loaded
//     uint32_t          frames; // total sample count
//     volatile uint32_t pos;    // playback cursor
//     volatile bool     active; // true = currently playing
// } voice_t;
 
// static voice_t voices[4];   // 0=A  1=B  2=C  3=D
 
// // ---------------------------------------------------------------------------
// // Ping-pong buffers + shared flags
// // ---------------------------------------------------------------------------
// static uint16_t  bufA[BUF_SAMPLES];
// static uint16_t  bufB[BUF_SAMPLES];
 
// // buf_need[b]  – core0 ISR tells core1 "buffer b needs a refill"
// // buf_ready[b] – core1 tells core0 ISR "buffer b is freshly filled"
// // playing      – which buffer is currently streaming (false=A, true=B)
// static volatile bool buf_need[2];
// static volatile bool buf_ready[2];
// static volatile bool playing;      // false=A, true=B
// static volatile bool need_refill;  // set by ISR, cleared by core1
 
// // ---------------------------------------------------------------------------
// // PWM / DMA globals
// // ---------------------------------------------------------------------------
// static uint      pwm_slice;
// static uint      pwm_chan;
// static uint32_t  pwm_period;   // TOP + 1 = samples per PWM cycle
// static uint16_t  pwm_top;
// static int       dma_ch = -1;
// static volatile uint16_t *dma_dst; // CC register halfword
 
// // ---------------------------------------------------------------------------
// // Forward declarations
// // ---------------------------------------------------------------------------
// static void core1_worker(void);
// static void dma_isr(void);
 
// // ---------------------------------------------------------------------------

// // ---------------------------------------------------------------------------
// // fill_buf – mix active voices into one ping-pong buffer
// // Called from core1 only.
// // ---------------------------------------------------------------------------
// static void fill_buf(uint16_t *dst, uint32_t n)
// {
//     for (uint32_t i = 0; i < n; i++) {
//         int32_t mix = 0;
//         int count = 0;
 
//         for (int v = 0; v < 4; v++) {
//             voice_t *vo = &voices[v];
//             if (!vo->active || vo->pos >= vo->frames) continue;
 
//             mix += vo->pcm[vo->pos++];
//             count++;
 
//             if (vo->pos >= vo->frames) {
//                 vo->active = false;

//                 vo->frames = 0; //MIGHT NEED TO KILL THIS LINE
//             }
//         }
 
//         if (count > 1)   mix /= count;
//         if (mix >  32767) mix = 32767;
//         if (mix < -32768) mix = -32768;
 
//         // Signed PCM → unsigned → scale to PWM range [0, pwm_period)
//         uint32_t u = (uint32_t)(mix + 32768);          // 0..65535
//         uint32_t level = (u * pwm_period) >> 16;
//         if (level >= pwm_period) level = pwm_period - 1;
//         dst[i] = (uint16_t)level;
//     }
// }
 
// // ---------------------------------------------------------------------------
// // init_audio – configure PWM slice + DMA channel
// // ---------------------------------------------------------------------------
// static void init_audio(void)
// {
//     gpio_set_function(AUDIO_GPIO, GPIO_FUNC_PWM);
//     pwm_slice = pwm_gpio_to_slice_num(AUDIO_GPIO);
//     pwm_chan  = pwm_gpio_to_channel(AUDIO_GPIO);
 
//     // Derive TOP directly from system clock so math is always correct
//     uint32_t sys_hz = clock_get_hz(clk_sys);
//     uint32_t top = (sys_hz / RATE) - 1u;
//     pwm_top = (uint16_t)top;
//     pwm_period = (uint32_t)top + 1u;
 
//     printf("[audio] sys_hz=%lu RATE=%u top=%lu period=%lu\n",
//            (unsigned long)sys_hz, RATE,
//            (unsigned long)pwm_top, (unsigned long)pwm_period);
 
//     pwm_set_clkdiv_int_frac(pwm_slice, 1, 0);  // divider = 1
//     pwm_set_wrap(pwm_slice, pwm_top);
//     pwm_set_chan_level(pwm_slice, pwm_chan, (uint16_t)(pwm_period / 2)); // silence
//     pwm_set_enabled(pwm_slice, true);
 
//     // DMA: 16-bit transfers, paced by PWM DREQ, write to CC register
//     dma_ch = dma_claim_unused_channel(true);
//     dma_channel_config cfg = dma_channel_get_default_config(dma_ch);
//     channel_config_set_transfer_data_size(&cfg, DMA_SIZE_16);
//     channel_config_set_read_increment(&cfg,  true);
//     channel_config_set_write_increment(&cfg, false);
//     channel_config_set_dreq(&cfg, pwm_get_dreq(pwm_slice));
 
//     // Point at the correct 16-bit halfword of the CC register
//     volatile uint16_t *cc16 = (volatile uint16_t *)&pwm_hw->slice[pwm_slice].cc;
//     dma_dst = (pwm_chan == PWM_CHAN_A) ? &cc16[0] : &cc16[1];
 
//     dma_channel_set_irq0_enabled(dma_ch, true);
//     irq_set_exclusive_handler(DMA_IRQ_0, dma_isr);
//     irq_set_enabled(DMA_IRQ_0, true);
 
//     dma_channel_configure(dma_ch, &cfg, (void *)dma_dst, bufA, BUF_SAMPLES, false /* don't start yet */);
// }
 
// // ---------------------------------------------------------------------------
// // DMA completion ISR – runs on core0, swaps buffers
// // ---------------------------------------------------------------------------
// static void dma_isr(void)
// {
//     dma_hw->ints0 = 1u << dma_ch;
 
//     // Mark the buffer we just finished as needing refill
//     buf_need[(int)playing] = true;
 
//     // Switch to the other buffer if core1 has it ready
//     bool next = !playing;
//     if (buf_ready[(int)next]) {
//         buf_ready[(int)next] = false;
//         playing = next;
//         dma_channel_set_read_addr(dma_ch, next ? (void *)bufB : (void *)bufA, true /* trigger */);
//     }
//     // If next isn't ready yet, DMA stops — core1's stall-restart handles it
 
//     need_refill = true;
// }
 
// // ---------------------------------------------------------------------------
// // core1_worker – fills ping-pong buffers and restarts stalled DMA
// // ---------------------------------------------------------------------------
// static void core1_worker(void)
// {
//     // Pre-fill both buffers with silence and signal core0
//     uint16_t silence = (uint16_t)(pwm_period / 2);
//     for (uint32_t i = 0; i < BUF_SAMPLES; i++) { bufA[i] = silence; bufB[i] = silence; }
 
//     buf_ready[0] = true;
//     buf_ready[1] = true;
 
//     multicore_fifo_push_blocking(0xCAFE);  // tell core0 we're ready
 
//     while (true) {
//         // Spin-wait for DMA ISR to signal a refill is needed
//         while (!need_refill) tight_loop_contents();
//         need_refill = false;
 
//         for (int b = 0; b < 2; b++) {
//             if (buf_need[b]) {
//                 buf_need[b] = false;
//                 fill_buf(b == 0 ? bufA : bufB, BUF_SAMPLES);
//                 buf_ready[b] = true;
//             }
//         }
 
//         // If DMA stalled (next buffer wasn't ready in time), restart it
//         if (!dma_channel_is_busy(dma_ch)) {
//             for (int b = 0; b < 2; b++) {
//                 if (buf_ready[b]) {
//                     buf_ready[b] = false;
//                     playing      = (bool)b;
//                     dma_channel_set_read_addr(dma_ch, b ? (void *)bufB : (void *)bufA, true);
//                     break;
//                 }
//             }
//         }
//     }
// }
 
// // ---------------------------------------------------------------------------
// // trigger_voice – decode on first press, retrigger on subsequent presses
// // ---------------------------------------------------------------------------
 
// // These structs map keypad letters to their MP3 asset data.
// // *** If you see "asset size: 0 bytes" in the terminal, the variable name
// //     in audio_assets.h does not match what is written here. Open
// //     audio_assets.h and copy the exact array name and length variable name
// //     into the table below. ***

 
// static void trigger_voice(int idx) {
//     static const int16_t * const flash_pcm[4] = {
//         kick_pcm, snare_pcm, hat_pcm, sad_sample_pcm
//     };
//     static const uint32_t flash_frames[4] = {
//         kick_pcm_len, snare_pcm_len, hat_pcm_len, sad_sample_pcm_len
//     };
//     voice_t *v = &voices[idx];
//     v->pcm = flash_pcm[idx];
//     v->frames = flash_frames[idx];
//     v->pos = 0;
//     __dmb();
//     v->active = true;
//     printf("[trig] voice %d: %lu frames\n", idx, (unsigned long)v->frames);
// }
 
// // ---------------------------------------------------------------------------
// // audio_machine – public entry point called from main.c
// // ---------------------------------------------------------------------------
// // void audio_machine(void) {
// //     // Populate asset table at runtime (len values are extern variables, not constants)
// //     keypad_init_pins();
// //     keypad_init_timer();
// //     display_init_pins();
// //     display_init_timer();
 
// //     init_audio();
 
// //     printf("[dm] Launching core1...\n");
// //     multicore_launch_core1(core1_worker);
// //     multicore_fifo_pop_blocking();   // wait for core1 ready
// //     printf("[dm] Core1 ready. pwm_period=%lu\n", (unsigned long)pwm_period);
 
    
 
// //     // Start DMA on buffer A (core1 already filled it with silence)
// //     buf_ready[0] = false;
// //     playing = false;
// //     dma_channel_set_read_addr(dma_ch, bufA, true /* trigger */);
// //     printf("[dm] DMA started. Ready — press A / B / C / D.\n");
 
// //     // Core0 main loop: consume keypad events
// //     while (true) {
// //         uint16_t evt = key_pop();
// //         bool pressed = (evt >> 8) & 1u;
// //         char k = (char)(evt & 0xFF);
 
// //         if (pressed) {
// //             // Show key on 7-segment display
// //             char disp[9] = "        ";
// //             disp[0] = k;
// //             display_char_print(disp);
 
// //             switch (k) {
// //                 case 'A': trigger_voice(0); break;
// //                 case 'B': trigger_voice(1); break;
// //                 case 'C': trigger_voice(2); break;
// //                 case 'D': trigger_voice(3); break;
// //                 default:  break;
// //             }
// //         }
// //     }
// // }

// void audio_machine_init(void) {
//     display_init_pins();
//     display_init_timer();
//     init_audio();
//     multicore_launch_core1(core1_worker);
//     multicore_fifo_pop_blocking();
//     buf_ready[0] = false;
//     playing      = false;
//     dma_channel_set_read_addr(dma_ch, bufA, true);
// }

// void play_kick(void)   { trigger_voice(0); }
// void play_snare(void)  { trigger_voice(1); }
// void play_hat(void)    { trigger_voice(2); }
// void play_sad_sample(void) { trigger_voice(3); }