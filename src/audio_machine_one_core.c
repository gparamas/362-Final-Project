// audio_machine.c
// Single-core audio engine: PWM wrap ISR fires at 44100 Hz,
// mixes up to 4 voices, writes one sample to the CC register.
...
// ---------------------------------------------------------------------------
// Voice state — written by core0, read inside the PWM ISR (also core0)
// ---------------------------------------------------------------------------
typedef struct {
    const int16_t   *pcm;    // points into flash
    uint32_t         frames;
...
// Flash PCM tables — indexed by voice number
...
// ---------------------------------------------------------------------------
// PWM wrap ISR — fires at RATE Hz, mixes voices, writes one sample
// ---------------------------------------------------------------------------
static void audio_isr(void)
{
    ...
    // Signed int16 → unsigned → scale to [0, pwm_period)
...
// ---------------------------------------------------------------------------
// audio_machine_init — call once before any play_* function
// ---------------------------------------------------------------------------
void audio_machine_init(void)
{
    ...
    uint32_t top    = (sys_hz / RATE) - 1u;   // 3400 at 150 MHz / 44100 Hz
    pwm_period      = top + 1u;
